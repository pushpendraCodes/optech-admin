...
import { ExamAttemptResultModal, type ExamAttemptRow } from "./ExamAttemptResultModal";
...
export function ExamsPage() {
  ...
  const [viewAttempt, setViewAttempt] = useState<ExamAttemptRow | null>(null);

  const extra: Record<string, string> = {};
  if (courseFilter) extra.course = courseFilter;
  if (openFilter) extra.open = openFilter;

  const { data, isLoading, isError, refetch } = useListQuery({ resource: "exams", page, search: debounced, extra });
  const attempts = useListQuery({ resource: "exam-attempts", page: attemptsPage, limit: 20 });
  ...
  const attemptRows = (attempts.data?.data ?? []) as ExamAttemptRow[];
  const attemptsMeta = attempts.data?.meta;

  function attemptStudentName(a: ExamAttemptRow) {
    return a.student?.user?.name || a.student?.studentCode || "—";
  }

  function attemptExamTitle(a: ExamAttemptRow) {
    return a.exam?.title ?? "—";
  }

  function attemptCourse(a: ExamAttemptRow) {
    return a.exam?.course ? loc(a.exam.course.title) : "—";
  }

  function attemptPassed(a: ExamAttemptRow) {
    if (a.status === "in_progress") return null;
    const passing = Number(a.exam?.passing ?? 0);
    return Number(a.percent ?? 0) >= passing;
  }

  async function togglePublish(id: string, publish: boolean) {
    try {
      await act({ path: `exams/${id}/${publish ? "publish" : "unpublish"}` }).unwrap();
      toast(publish ? "Exam published" : "Exam unpublished");
      refetch();
    } catch {
  ...
  return (
    <div>
      <PageHeader
        title="Exams"
        description="Create timed course exams, publish them for enrolled students, and pull questions from the shared bank."
        actions={
          canWrite && tab === "tests" ? (
            <Button type="button" onClick={() => { setEditorId(null); setEditorOpen(true); }}>New exam</Button>
          ) : null
        }
      />

      <div className="mb-6 flex gap-2">
        <Button type="button" variant={tab === "tests" ? "primary" : "ghost"} onClick={() => setTab("tests")}>Exams</Button>
      ...
      {tab === "bank" ? <QuestionBankPanel /> : (
        <>
          ...
          {isLoading ? (
            ...
            <EmptyState title="No Exams" body="Create a test with questions from manual entry or the question bank." />
          ) : (
                    ...
                    {attemptRows.map((a) => {
                      ...
                      return (
                        <tr key={String(a._id)} className="border-b border-white/5">
                          ...
                          <td className="px-3 py-2">{attemptExamTitle(a)}</td>
      ...
      <ExamAttemptResultModal
        open={Boolean(viewAttempt)}
        row={viewAttempt}
        onClose={() => setViewAttempt(null)}
        onCertificateIssued={({ attemptId, enrollmentId, certificate }) => {
          setViewAttempt((prev) =>
            prev && String(prev._id) === attemptId
              ? { ...prev, enrollmentId, certificate }
              : prev,
          );
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (!removeId) return;
          try {
            await remove({ resource: "exams", id: removeId }).unwrap();
            toast("Test deleted");
            setRemoveId(null);
            refetch();
          } catch {
